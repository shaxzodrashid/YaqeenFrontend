...
| `origin_place`              | `VARCHAR(255)`  | `NULLABLE`                                            | Origin loading station/city (e.g., `Beijing`, `Istanbul`)                                                 |
| `destination_place`         | `VARCHAR(255)`  | `NULLABLE`                                            | Final delivery city/customs (e.g., `Tashkent`)                                                            |
...
Base URL: `/api/cargo-consolidations` (All routes require `JwtAuthGuard` and `PermissionsGuard`).

| Method   | Endpoint                                      | Permission                    | Description                                                                             |
| :------- | :-------------------------------------------- | :---------------------------- | :-------------------------------------------------------------------------------------- |
| `POST`   | `/api/cargo-consolidations`                   | `cargo_consolidations:create` | Creates a new consolidation truck trip                                                  |
| `GET`    | `/api/cargo-consolidations`                   | `cargo_consolidations:read`   | Paginated list with search, status filters, utilization %, and financials               |
| `GET`    | `/api/cargo-consolidations/active`            | `cargo_consolidations:read`   | **Active dropdown endpoint** for frontend Search-or-Create picker                       |
| `GET`    | `/api/cargo-consolidations/:id`               | `cargo_consolidations:read`   | Full consolidation details with attached client cargos list                             |
| `PATCH`  | `/api/cargo-consolidations/:id`               | `cargo_consolidations:update` | Updates consolidation (with optional cascade sync to attached cargos)                   |
| `POST`   | `/api/cargo-consolidations/:id/assign-cargos` | `cargo_consolidations:update` | Batch assigns cargo registrations to this consolidation                                 |
| `POST`   | `/api/cargo-consolidations/:id/remove-cargos` | `cargo_consolidations:update` | Batch unlinks cargo registrations from this consolidation                               |
| `DELETE` | `/api/cargo-consolidations/:id`               | `cargo_consolidations:delete` | Deletes consolidation record (safely resets attached cargos' `consolidation_id = NULL`) |

---

## 5. Payloads and Response Schemas

### 5.1. Create Consolidation (`POST /api/cargo-consolidations`)
