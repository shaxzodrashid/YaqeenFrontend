    ...
    <div className="flex flex-col gap-2 w-full text-left">
      ...
      >
        ...
        >
          ...
          <ChevronDown
            className={`size-3.5 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''} text-neutral-600 dark:text-neutral-400`}
          />
        </button>
            ...
            {countries.map((c) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="text-base" role="img" aria-label={c.nameKey}>
                    {c.flag}
                  </span>
                  <span>{c.nameKey}</span>
                </div>
      ...
      {error && (
        <span className="text-xs text-danger animate-in fade-in duration-200">{error}</span>
      )}
    </div>
  );
}
