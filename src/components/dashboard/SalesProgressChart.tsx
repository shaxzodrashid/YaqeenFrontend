...
export const SalesProgressChart: React.FC<SalesProgressChartProps> = React.memo(({ data, loading }) => {
  ...
  if (loading) {
    return (
      <div className="p-6 rounded-2xl bg-surface dark:bg-night-surface border border-border/60 dark:border-night-border h-[400px] flex items-center justify-center animate-pulse">
        <div className="text-xs text-muted"><T k="loading" text="Loading Analytics..." /></div>
      </div>
    );
  }
  ...
  if (!data || !data.dataPoints || data.dataPoints.length === 0) {
    return (
      <div className="p-8 rounded-2xl bg-surface dark:bg-night-surface border border-border/60 dark:border-night-border h-[400px] flex flex-col items-center justify-center gap-2 text-center">
        ...
        <p className="text-sm font-semibold text-foreground dark:text-night-text"><T k="ovChartNoDataTitle" /></p>
        <p className="text-xs text-muted"><T k="ovChartNoDataDesc" /></p>
      </div>
    );
  }
  ...
  const maxVal = Math.max(
    ...dataPoints.map((d) => (mode === 'period' ? Math.max(d.sales, d.margin) : Math.max(d.cumulativeSales, d.cumulativeMargin))),
    100
  );
    ...
    <div className="flex flex-col gap-4 p-6 rounded-2xl bg-surface dark:bg-night-surface border border-border/60 dark:border-night-border shadow-sm transition-all duration-300">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <h3 className="text-base font-bold text-foreground dark:text-night-text"><T k="ovChartSalesProgress" /></h3>
        ...
        <div className="flex items-center gap-1 p-1 bg-border/20 dark:bg-night-border/40 rounded-xl">
          ...
          >
            ...
            <span><T k="ovChartPeriodSales" /></span>
          </button>
          <button
          ...
          >
            ...
            <span><T k="ovChartCumulative" /></span>
          </button>
        </div>
      </div>
      ...
      <div className="relative w-full overflow-hidden">
        ...
        >
          ...
          {/* Sales Line */}
          <path d={salesPath} fill="none" stroke="#2563eb" strokeWidth={3} strokeLinecap="round" />
                ...
                {isHovered && (
                  <>
                    <line
                      ...
                      strokeDasharray="2 2"
                    />
                    <circle cx={p.x} cy={p.ySales} r={5} fill="#2563eb" stroke="#ffffff" strokeWidth={2} />
                    <circle cx={p.x} cy={p.yMargin} r={4} fill="#10b981" stroke="#ffffff" strokeWidth={2} />
                  </>
                )}
              </g>
            );
        ...
        {hoverPoint && (
          <div className="absolute top-2 right-4 p-3 rounded-xl bg-surface/95 dark:bg-night-surface/95 border border-border/80 dark:border-night-border shadow-lg backdrop-blur-md text-xs pointer-events-none min-w-[200px] animate-fadeIn">
            <div className="flex items-center justify-between font-bold border-b border-border/40 pb-1.5 mb-2">
              <span className="text-foreground dark:text-night-text">{hoverPoint.dataPoint.label}</span>
            ...
            <div className="flex flex-col gap-1">
              <div className="flex items-center justify-between text-blue-600 dark:text-blue-400">
                ...
                <span className="font-bold">
                  {formatMoney(
                    mode === 'period' ? hoverPoint.dataPoint.sales : hoverPoint.dataPoint.cumulativeSales,
                    'USD'
                  )}
                </span>
              </div>
              <div className="flex items-center justify-between text-purple-500">
                ...
                <span className="font-medium">{formatMoney(hoverPoint.dataPoint.purchaseCost, 'USD')}</span>
              </div>
              <div className="flex items-center justify-between text-emerald-500">
                <span>{mode === 'period' ? 'Net Margin:' : 'Cumulative Margin:'}</span>
                <span className="font-bold">
                  {formatMoney(
                    mode === 'period' ? hoverPoint.dataPoint.margin : hoverPoint.dataPoint.cumulativeMargin,
                    'USD'
                  )}
                </span>
              </div>
              <div className="flex items-center justify-between text-muted text-[11px] pt-1 border-t border-border/30">
                ...
                <span className="font-semibold text-foreground">{hoverPoint.dataPoint.orderCount} orders</span>
              </div>
            </div>
          </div>
        )}
